...
1. **Channels** (`selmakit.channels.WebChatChannel`, `TelegramChannel`)
   ...
   **Which files to attach and which *form* of them to send are separate decisions.** The scan sends exactly the files the answer names; the *form* is the channel's call, because only the channel knows what it can display — Telegram's file viewer runs no JavaScript, so a folium map arrives as a blank white page, while the dashboard embeds the same HTML fine. So each channel declares a table (`TelegramReply.RENDERABLE_SUFFIXES` / `SUBSTITUTE_SUFFIXES`) and `pair_renderable_siblings()` uses it: for an artefact the channel cannot render, a *same-directory, same-stem* sibling in a renderable format (`map.png` beside `map.html`) is sent too, picture first, the original still following. The application decides which forms of its artefact exist — writing the `.png` next to the `.html` is independently useful, a vision model cannot read HTML either — the channel picks among them, and **the model is told about neither**; it just names what it produced. The sibling passes the same containment check as any other candidate and is never sent twice. No sibling, or a channel that declares HTML renderable (web chat substitutes nothing): unchanged behaviour.
...
selmakit was originally built against pydantic-ai 1.94.0. Migration to 2.0 (beta) reshaped the architecture (the project now tracks **2.24.x**; 2.22 added the deferred-tools API that [Tool approval](#tool-approval-deferred-tools) builds on):
